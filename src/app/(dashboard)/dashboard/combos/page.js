      ...
      {combos.length === 0 ? (
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex min-w-0 flex-col gap-2 rounded-lg border border-black/5 bg-black/[0.015] px-3 py-2 dark:border-white/5 dark:bg-white/[0.02] sm:flex-row sm:items-center sm:justify-between">
            <label className={`flex items-center gap-2 text-xs text-text-muted select-none ${canWrite ? "cursor-pointer hover:text-primary" : "cursor-not-allowed opacity-50"}`}>
              <input
                type="checkbox"
                checked={allSelected}
                ref={(el) => {
              ...
              />
              <span>{someSelected ? `${selectedIds.length} selected` : `Select all (${combos.length})`}</span>
            </label>
            ...
            <div className="flex min-w-0 flex-wrap items-center gap-2">
              {someSelected && (
                <>
                  ...
                  <Button size="sm" variant="danger" icon="delete" disabled={bulkBusy || !canWrite} loading={bulkBusy} onClick={handleBulkDelete} className="whitespace-nowrap" title={!canWrite ? "Admin only" : undefined}>
...
function ComboCard({ combo, getCaps, comboByName = {}, activeProviders = [], copied, onCopy, onEdit, onDelete, strategy = {}, onSetStrategy, canWrite, selected = false, onToggleSelect }) {
