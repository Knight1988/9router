...
describe("compatible provider connections API", () => {
  ...
  it("rejects a second connection on the same compatible node", async () => {
    const ctx = await setupTestContext({
    ...
    const makeRequest = (provider) => new Request("https://9router.local/api/providers", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ provider, apiKey: "test-key", name: "Connection 1", defaultModel: "test-model" }),
    });
