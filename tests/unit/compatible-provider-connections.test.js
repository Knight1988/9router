...
describe("compatible provider connections API", () => {
  ...
  it("creates one API-key connection for an OpenAI-compatible node", { timeout: 15000 }, async () => {
  ...
  it("allows multiple connections on the same compatible node", async () => {
    ...
    const req1 = new Request("https://9router.local/api/providers", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ provider: ctx.node.id, apiKey: "test-key-1", name: "Connection 1", defaultModel: "test-model" }),
    });
    const req2 = new Request("https://9router.local/api/providers", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ provider: ctx.node.id, apiKey: "test-key-2", name: "Connection 2", defaultModel: "test-model" }),
    });
    const firstResponse = await ctx.POST(req1);
    const secondResponse = await ctx.POST(req2);
    const storedConnections = await ctx.getProviderConnections({ provider: ctx.node.id });
