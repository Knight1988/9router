...
import { proxyAwareRawFetch } from "../../open-sse/utils/proxyFetch.js";

vi.mock("../../open-sse/utils/proxyFetch.js", () => ({
  proxyAwareRawFetch: vi.fn(async () => ({ ok: true, status: 200, headers: { get: () => "" } })),
}));
...
function expectOriginalToolsPreserved(tools) {
...
describe("opencode Free 1.3 tool_choice auto-only", () => {
  ...
  it.each([
    ...
    for (const model of [FREE_13, `${FREE_13}(max)`]) {
      ...
      expectOriginalToolsPreserved(out.tools);
      expect(out.input).toEqual(INPUT);
    }
  });

  it("giữ auto và defaults absent sang auto; tools/input nguyên vẹn", () => {
    ...
    expectOriginalToolsPreserved(autoOut.tools);
    ...
    expectOriginalToolsPreserved(absentOut.tools);
  ...
  it("wire: execute gửi choice auto tới /zen/v1/responses", async () => {
    ...
    expect(proxyAwareRawFetch).toHaveBeenCalledTimes(1);
    const [actualUrl, actualInit] = proxyAwareRawFetch.mock.calls[0];
    ...
    expectOriginalToolsPreserved(sent.tools);
