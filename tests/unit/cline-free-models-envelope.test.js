...
describe("cline free-models envelope in nonStreamingHandler", () => {
  ...
  // The unwrap is opt-in via transport.quirks.clineEnvelope, so another
  // provider's envelope is not treated as a valid OpenAI completion. The
  // handler's normal empty-completion guard must reject it rather than leak
  // or silently rewrite the provider-specific body.
  it("rejects an enveloped body for a provider that did not opt in", async () => {
    const providerResponse = new Response(
      JSON.stringify({
        success: true,
    ...
    const body = await result.response.json();
    expect(body.error.message).toContain("Empty completion from openai/z-ai/glm-5.3-flash");
  });
});
