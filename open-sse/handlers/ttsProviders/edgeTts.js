...
async function getToken() {
  ...
  const { result: res } = await fetchWithRetry("https://www.bing.com/translator", {
...
async function ttsRequest(text, voiceId, token) {
  ...
  const { result } = await fetchWithRetry(
    ...
    // Retry on transient errors and 429/403 (token expiry handled by cache invalidation below)
    { maxRetries: 2, baseDelay: 1000, retryOnStatus: [502, 503, 504] }
  );
  return result;
}
...
export default {
  ...
  async synthesize(text, model) {
    ...
    // 429/403: invalidate cache and retry once with fresh token
